...
async function testUpsert() {
    ...
    const { data, error } = await supabase
        ...
        .upsert({ key: 'slogan', value: 'New Slogan Test' }, { onConflict: 'key' });

    if (error) {
        console.error("UPSERT ERROR:", error);
    } else {
        console.log("UPSERT SUCCESS (or silenced by RLS):", data);

        // Check again if it was actually saved
        const { data: checkData } = await supabase.from('system_settings').select('*').eq('key', 'slogan');
