            ...
            <section className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-700">
                ...
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 bg-white/5 backdrop-blur-2xl p-10 rounded-[2.5rem] border border-white/10">
                    <div className="space-y-2">
                        <h2 className="text-4xl md:text-6xl font-black tracking-tight text-[#3B7552] uppercase">Reservas</h2>
                        <p className="text-xl text-[#3B7552] font-bold uppercase tracking-widest pl-1">Agenda de Entrenamiento</p>
                    </div>
                    ...
                    <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                        ...
                        <DialogContent className="sm:max-w-[425px]">
                            <DialogHeader>
                                <DialogTitle className="text-2xl font-black uppercase tracking-tight text-[#3B7552]">Nueva Reserva</DialogTitle>
                                <DialogDescription className="text-xs font-bold uppercase tracking-widest text-[#3B7552]/40">Asignación de turno para usuario.</DialogDescription>
                            </DialogHeader>
                            <div className="grid gap-4 py-4">
                                <div className="grid gap-2">
                                    <Label className="text-[#3B7552]">Usuario</Label>
                                    <Select value={selectedMember} onValueChange={setSelectedMember}>
                                        <SelectTrigger className="border-[#3B7552]/10"><SelectValue placeholder="Selecciona un usuario" /></SelectTrigger>
                                        <SelectContent>
                            ...
                            <DialogFooter>
                                <Button onClick={handleCreateReservation} className="bg-[#3B7552] text-white font-black uppercase tracking-widest h-12 rounded-xl px-8 shadow-lg shadow-[#3B7552]/20 hover:scale-105 active:scale-95 transition-all">Confirmar Reserva</Button>
                            </DialogFooter>
                        </DialogContent>
                    </Dialog>
                </div>
                ...
                <Card className="border-white/10 bg-white/5 backdrop-blur-xl rounded-[2.5rem] shadow-2xl">
                    <CardHeader className="pb-4">
                        <CardTitle className="text-2xl font-black text-[#3B7552] flex items-center gap-3">
                            <Calendar className="h-6 w-6 text-[#3B7552]" />
                            Selecciona tu Fecha
                        </CardTitle>
                    </CardHeader>
                    <CardContent className="flex flex-col md:flex-row gap-8">
                        <div className="space-y-3">
                            <Label className="text-[#3B7552]/60 text-[10px] font-black uppercase tracking-widest pl-1">Día del Entrenamiento</Label>
                            <input
                                type="date"
                                value={selectedDate}
                                onChange={e => setSelectedDate(e.target.value)}
                                className="w-full md:w-72 h-14 px-6 rounded-2xl bg-white/5 border border-[#3B7552]/10 text-[#3B7552] font-bold text-lg focus:ring-2 focus:ring-[#3B7552]/50 outline-none transition-all uppercase"
                            />
                        </div>

                        <div className="space-y-3 flex-1">
                            <Label className="text-[#3B7552]/60 text-[10px] font-black uppercase tracking-widest pl-1">Área de Entrenamiento</Label>
                            <div className="grid grid-cols-2 gap-3 p-1.5 bg-white/5 rounded-2xl border border-[#3B7552]/10 h-14">
                                <button
                                    onClick={() => setSelectedArea("gym")}
                                    className={`rounded-xl font-black uppercase text-xs tracking-wider transition-all ${selectedArea === "gym" ? "bg-[#3B7552] text-white shadow-lg" : "text-[#3B7552]/40 hover:text-[#3B7552]/60"}`}
                                >
                                    Gimnasio
                                </button>
                                <button
                                    onClick={() => setSelectedArea("cognitive")}
                                    className={`rounded-xl font-black uppercase text-xs tracking-wider transition-all ${selectedArea === "cognitive" ? "bg-[#3B7552] text-white shadow-lg" : "text-[#3B7552]/40 hover:text-[#3B7552]/60"}`}
                                >
                                    Área Cognitiva
                                </button>
                            </div>
                ...
                <div className="space-y-8">
                    <h3 className="text-2xl font-black text-[#3B7552] px-2 flex items-center gap-3 uppercase tracking-tight">
                        <Clock className="h-6 w-6 text-[#3B7552]" />
                        Turnos de Hoy
                    </h3>
                                    ...
                                    <CardContent className="p-8 space-y-6">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                <div className="p-2.5 rounded-xl bg-[#3B7552]/10 border border-[#3B7552]/20">
                                                    <Clock className="h-5 w-5 text-[#3B7552]" />
                                                </div>
                                                <span className="text-2xl font-black text-[#3B7552]">{slot.time}</span>
                                            </div>
                                            <div className={`w-3 h-3 rounded-full ${status.color === 'bg-green-500' ? 'bg-[#3B7552]' : status.color} shadow-[0_0_10px_currentcolor] animate-pulse`} />
                                        </div>

                                        <div className="space-y-3">
                                            <div className="flex items-center justify-between text-xs font-bold uppercase tracking-widest">
                                                <span className="text-[#3B7552]/40">Cupos Ocupados</span>
                                                <span className="text-[#3B7552]">{slot.booked} / {slot.capacity}</span>
                                            </div>
                                        ...
                                        <div className="space-y-3 pt-4 border-t border-white/5">
                                            {slotReservations.map(reservation => (
                                                <div key={reservation.id} className="flex items-center justify-between group/user p-3 rounded-xl bg-white/[0.02] hover:bg-white/5 transition-all">
                                                    <div className="flex items-center gap-3">
                                                        <div className="w-8 h-8 rounded-full bg-[#3B7552]/10 border border-[#3B7552]/20 flex items-center justify-center text-[10px] font-black text-[#3B7552] uppercase">
                                                            {reservation.memberName.charAt(0)}
                                                        </div>
                                                        <span className="text-xs font-bold text-[#3B7552]/70 group-hover/user:text-[#3B7552] uppercase tracking-tight transition-colors">{reservation.memberName}</span>
                                                    </div>
                                                    <Button
                                                        variant="ghost"
                                                        size="icon"
