            ...
            <DashboardShell>
                <div className="flex items-center justify-center min-h-[400px]">
                    <div className="flex flex-col items-center gap-4">
                        <div className="w-12 h-12 border-4 border-[#3B7552] border-t-transparent rounded-full animate-spin" />
                        <p className="font-black text-[#3B7552]/40 uppercase tracking-widest text-xs">Cargando base de datos...</p>
                    </div>
                </div>
            </DashboardShell>
        )
            ...
            <section className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 bg-card p-10 rounded-[3rem] border border-border/10">
                    ...
                    <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
                        ...
                        <DialogContent className="sm:max-w-[425px]">
                            ...
                            <DialogFooter>
                                <Button onClick={handleCreate} disabled={isLoading} className="w-full h-12 rounded-xl font-bold bg-[#3B7552] text-white hover:bg-[#3B7552]/90">
                                    {isLoading ? (
                                        <>
                ...
                <div className="flex flex-col md:flex-row gap-6 bg-white/5 backdrop-blur-3xl p-8 rounded-[2.5rem] border border-white/10 shadow-2xl">
                    <div className="relative flex-1 group">
                        <Search className="absolute left-8 top-1/2 -translate-y-1/2 h-6 w-6 text-[#3B7552]/20 group-hover:text-[#3B7552] transition-colors" />
                        <Input
                            placeholder="Buscar por nombre, email..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="h-16 pl-20 pr-10 rounded-2xl bg-white/[0.03] border-white/10 text-lg font-bold focus-visible:ring-primary/20 transition-all text-white"
                        />
                    </div>
                ...
                <div className="bg-white/5 backdrop-blur-3xl rounded-[3.5rem] border border-white/10 shadow-2xl overflow-hidden">
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="border-b border-white/5 bg-white/[0.02]">
                                    <th className="px-12 py-8 font-black uppercase tracking-[0.2em] text-white/20 text-[10px]">Usuario</th>
                                    ...
                                    <tr key={member.id} className="group hover:bg-white/[0.03] transition-all duration-300">
                                        ...
                                        <td className="px-12 py-10">
                                            <button onClick={async () => await toggleMemberStatus(member.id)} className="flex items-center gap-4 hover:scale-105 transition-transform">
                                                <div className={`w-3 h-3 rounded-full ${member.status === 'Activo' ? 'bg-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.3)]' : 'bg-red-500 shadow-[0_0_15px_#ef4444]'} animate-pulse`} />
                                                <span className="font-black uppercase tracking-widest text-[10px] text-emerald-400">{member.status}</span>
                                            </button>
                                        </td>
                    ...
                    <div className="p-12 border-t border-white/5 bg-white/[0.01] text-center">
                        <p className="text-white/20 font-black uppercase tracking-[0.3em] text-xs">Total de {filteredMembers.length} Usuarios de Élite</p>
                    </div>
                </div>
            </section>
        </DashboardShell>
